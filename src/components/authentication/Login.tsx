...
import { auth_login, base_url, auth_glogin } from "../../api/route";
...
export default function Login({ registerSwap }: RegisterProps) {
  ...
  const [g_username, setG_username] = useState<string>("");
  const [openAuthPopup, setOpenAuthPopup] = useState<boolean>(false);
  const [thirdPartyAuth, setThirdpartyAuth] = useState<boolean>(false);
  ...
  const closePopup = () => {
    ...
    setG_username("");
    setThirdpartyAuth(false);
  };

  const handleThirdPartyLoginBtnClick = () => {
  ...
  const googleAuth = () => {
    if (thirdPartyAuth && openAuthPopup) {
      const url = `${base_url}${auth_glogin}?username=${g_username}`;
    ...
    <>
      ...
      >
        {openAuthPopup ? (
          ...
          >
            <Popup
              inputchange={setG_username}
              close={closePopup}
              username={g_username}
              continue={handleGoogleLogin}
              pending={isPending}
            />
          </motion.div>
        ) : (
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="googleLogin pb-3 w-full flex justify-center">
                  ...
                  >
                    ...
                    <span
                      className="text-lg"
                      onClick={handleThirdPartyLoginBtnClick}
                    >
                      Login with Google
                    </span>
                  </button>
